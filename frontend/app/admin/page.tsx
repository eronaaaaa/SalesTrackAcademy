    ...
    <main className="pt-8 pb-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-5 space-y-6 pt-6">
          ...
          <p className="text-4xl font-black mt-2 dark:text-white">{stats.averageScore}</p>
        </div> */}
          </section>
          ...
          className="group text-left bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-10 rounded-[3rem] hover:border-green-500 transition-all hover:shadow-2xl"
        >
          <div className="w-16 h-16 bg-green-100 dark:bg-green-900/30 rounded-2xl flex items-center justify-center text-3xl mb-6 group-hover:scale-110 transition-transform">
            📊
          </div>
          <h2 className="text-2xl font-black dark:text-white mb-2">Performance Reports</h2>
          <p className="text-slate-500 font-medium">Monitor quiz scores, track completion rates, and view agent analytics.</p>
        ...
        <React.Fragment key={agent.agentEmail}>
          ...
          >
            ...
            <td className="p-6 text-right">
              <span className={`transition-transform inline-block ${expandedAgent === agent.agentEmail ? 'rotate-180' : ''}`}>
                ↓
              </span>
            </td>
          </tr>
          ...
          {expandedAgent === agent.agentEmail && (
            <tr>
              <td colSpan={3} className="bg-slate-50/50 dark:bg-slate-800/20 p-6">
                    ...
                    <div key={idx} className="flex items-center justify-between bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                           ...
                           <span className="text-[10px] font-bold text-slate-400">{course.completionRate}</span>
                        </div>
                      </div>
