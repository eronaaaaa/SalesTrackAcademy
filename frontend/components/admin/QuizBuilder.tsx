...
export default function QuizBuilder({
  ...
  const handleSave = async () => {
    if (!questionText.trim() || choices.some((c) => !c.text.trim())) {
      return alert("Please fill in the question and all choices.");
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-[2rem] border border-slate-200 dark:border-slate-700 h-full max-h-[95vh]">
      <div className="flex flex-col min-h-0">
        <div className="flex items-baseline justify-between">
          <h3 className="text-xs font-black uppercase text-slate-400 tracking-tighter mb-4">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2">
                  {q.choices?.map((c: Choice) => (
                    <div
                      key={c.id}
                      className={`text-[10px] p-2 rounded-lg font-bold ${c.isCorrect ? "bg-green-100 text-green-700" : "bg-slate-50 dark:bg-slate-800 text-slate-400"}`}
                    >
                      {c.text}
                    </div>
                  ))}
      ...
      <div className="bg-blue-50/50 dark:bg-blue-900/10 p-6 rounded-[2.5rem] border border-blue-100 dark:border-blue-900/30">
        <h3 className="text-sm font-black dark:text-white mb-4">
          Add New Question
        </h3>
        <input
        ...
        <button
          onClick={handleSave}
          disabled={loading}
          className="w-full mt-6 py-4 bg-blue-600 text-white rounded-2xl font-black text-sm hover:scale-[1.02] transition-transform disabled:opacity-50"
        >
